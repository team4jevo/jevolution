 ...
 */
// when new game logic object is created graphic engine is passed to it in
// constructor
public class Controller {
    ...
    protected void handleNewGame(ActionEvent event) throws Exception {
        ...
        if (simulationInProgress) { // simulatiion is running. stop it and
    ...
    public void processTableGo() {
        tableObject = 0;
        colGoValue.setCellFactory(
                TextFieldTableCell.<GameParameter> forTableColumn());
    ...
    public ObservableList<GameParameter> getParam(
        ...
        ObservableList<GameParameter> gamePros = FXCollections
                .observableArrayList();
    ...
    private void setUpTable() {
        ...
        colGoValue.setOnEditCommit(
                new EventHandler<TableColumn.CellEditEvent<GameParameter, String>>() {
                    @Override
                    public void handle(
                            TableColumn.CellEditEvent<GameParameter, String> e) {
                        if (tableObject == 0) {
                            System.out.println("edditing go");
                            if (!tmpGoForTable.modifyStat(
                                    (String) colGoParameter.getCellData(
    ...
    public void initialize() {
        ...
        cbGameLogicSelector.setItems(
                FXCollections.observableArrayList("JevoSim", "simulation 2.2") // add
                                                                               // your
                                                                               // stuff
                                                                               // here
        );

        cbGameLogicSelector.getSelectionModel().selectedIndexProperty()
