...
public class Grid {
    private LocalGameObject[][] gameObjects;
    private boolean[][] nextState;
    private Random random;
    private HashMap<String, ArrayList<LocalCreature>> creaturesTotal; //TODO correct the way alive creatures are counted
    ...
    public Grid(int x, int y) throws Exception {
        ...
        this.gameObjects = new LocalGameObject[y][x];
     ...
     */
    void addObject(LocalGameObject object) throws Exception {
        if (!this.isFull()) {
     ...
     */
    public void addObject(LocalGameObject object, int x, int y) throws Exception {
    ...
    void printGrid() {
        for (int y = 0; y < this.getY(); y++) {
            for (int x = 0; x < this.getX(); x++) {
                LocalGameObject currObject = gameObjects[y][x];
                if (currObject == null) {
                    System.out.print(" ");
    ...
    public void update() {
        for (int y = 0; y < this.getY(); y++) {
            for (int x = 0; x < this.getX(); x++) {
                if (this.gameObjects[y][x] != null) {
                    LocalGameObject currObject = this.gameObjects[y][x];
                    ...
                    if (LocalCreature.class.isInstance(currObject)) {
                        ...
                        if (!cr.getType().equals(mostFrequent)) {
                            // Get corresponding aggressiveness of most common neighbor type
                            ...
                            // If creature is less aggressive than most frequent neighbors
                            if (creaturesAggressiveness < aggressiveness) {
                                ...
                                // If random value in range [0.0, 1.0) is less than probability
                                if (random.nextDouble() < p) {
                                    ...
                                    newCreature.setXCoord(x);
                                    newCreature.setYCoord(y);
                                    this.gameObjects[y][x] = newCreature;
                                    // Add new creature to total of such creatures
     ...
     */
    public LocalGameObject[][] getGameObjects() {
        return this.gameObjects;
    }

    /**
     * Method returns map with key as string that represents type of creature
     * and value as list that represents total number of creatures per
     * creature type.
     * 
        ...
        for (int i = 0; i < directions.size(); i++) {
            ...
            case 0:
                ...
                if (x != this.getX() - 1 && this.gameObjects[y][x + 1] != null) {
                    LocalGameObject currObject = this.gameObjects[y][x + 1];
            ...
            case 1:
                ...
                if (x < this.getX() - 1 && y > 0 && this.gameObjects[y - 1][x + 1] != null) {
                    LocalGameObject currObject = this.gameObjects[y - 1][x + 1];
                    if (!currObject.getType().equals("Food")) {
                        LocalCreature creature = (LocalCreature) currObject;
                        if (creature.getStatus()) {
                            ans.add(creature);
            ...
            case 2:
                ...
                if (y > 0 && this.gameObjects[y - 1][x] != null) {
                    LocalGameObject currObject = this.gameObjects[y - 1][x];
                    if (!currObject.getType().equals("Food")) {
                        LocalCreature creature = (LocalCreature) currObject;
                        if (creature.getStatus()) {
                            ans.add(creature);
