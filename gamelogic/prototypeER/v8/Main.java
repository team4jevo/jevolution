...
public class Main {
	 ...
	 * creatures do not get food often enough, they die of starvation.
	 */
	...
	public static void main(String[] args) {
		...
		for (int t = 0; t < time; t++) {
			...
			for (Creature creature : list_of_creatures) {
				...
				if (cx_1 != cx_0 || cy_1 != cy_0) {
					...
					case 2: {// creature
						...
						// creatures
							...
							if (temp_creature.strength < creature.strength) {
								...
								spawn_creature.spawn(10, 8, change_v_time, new_list_of_creatures, creature, field);
					...
					case 3: {// food
						...
						// trying to spawn a new creature
						spawn_creature.spawn(10, 9, change_v_time, new_list_of_creatures, creature, field);
						creature.cellXY(cx_1, cy_1);
						field.cell[cx_1][cy_1].change(2, creature);
						creature.locXY(x_1, y_1);
						break;
			...
			if (new_list_of_creatures.isEmpty() == false)
				list_of_creatures.addAll(new_list_of_creatures);
			...
			if (list_of_creatures_to_remove.isEmpty() == false) {
				...
				for (int i = 0; i < list_of_creatures.size(); i++)
					System.out.println("Creature " + i + " has eaten="
							+ list_of_creatures.get(i).food_eaten
							+ ", can eat creature?="
							+ list_of_creatures.get(i).eatCreature
							+ ", creatures eaten="
							+ list_of_creatures.get(i).creatures_eaten
							+ " and evolved "
							+ list_of_creatures.get(i).times_evolved
							+ " times.");
				// Not necessary
			}
		...
		for (Creature creature : all_creatures) {
			System.out.print("Creature " + a + "( id=" + creature.id
					...
					+ " times and birthed "+creature.creatures_spawned+ " creatures. ");
