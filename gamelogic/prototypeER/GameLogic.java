...
public class GameLogic extends jevo.GameLogic  {
			...
			try {
				...
				for (int i = 0; i < cx_length; i++)
					for (int j = 0; j < cy_length; j++) {
						...
						if (whatis != 0) {
							cell[i][j] = new Cell(whatis, "Placeholder", i, j);
						}
					}
				scan.nextLine();
				scan.nextLine();
	...
	public void nextTurn() {
		...
		for (Creature creature : list_of_alive_creatures) {
			...
			if(creature.eatCreature) c_color=Color.RED;
...
//					cell[cx_0][cy_0] = null;
					...
					cell[cx_1][cy_1].setToken(super.getGe().createToken(cell[cx_1][cy_1]));
					...
					case 3: {// food
						...
						cell[cx_1][cy_1].removeToken();
						cell[cx_1][cy_1] = null;
						cell[cx_1][cy_1] = new Cell(2, creature, "", cx_1,
								cy_1);
						cell[cx_1][cy_1].setToken(super.getGe()
								.createToken(cell[cx_1][cy_1]));
						cell[cx_1][cy_1].getToken().setColor(c_color);
